...
import json
import requests
from term_identification import assign_paragraph_to_topic, topics_keywords
...
def html_to_summary(url):
    ...
    for header in soup.find_all(header_pattern): 
        ...
        if assigned_topic not in my_dict:
            my_dict[assigned_topic] = curr_summary  # Create a new entry
        else:
            my_dict[assigned_topic] += curr_summary  # Append to existing entry

    # Sort the dictionary by keys, placing 'Other' last
    sorted_dict = {k: my_dict[k] for k in sorted(my_dict.keys(), key=lambda x: (x == 'Other', x))}
